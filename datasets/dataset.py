...
class PotholesDataset(Dataset):
    def __init__(self, dataset_path, class_name='potholes_roi', is_train=True, resize=256, cropsize=224):
        self.dataset_path = dataset_path
        self.class_name = class_name
        self.is_train = is_train
        self.resize = resize
        ...
        if is_train:
            ...
            self.transform_x = T.Compose([T.Resize([cropsize, cropsize], Image.ANTIALIAS),
                                          T.ToTensor(),
                                          T.Normalize(mean=[0.485, 0.456, 0.406],
                                                      std=[0.229, 0.224, 0.225])])
        self.transform_mask = T.Compose([T.Resize([cropsize, cropsize], Image.NEAREST),
                                         T.ToTensor()])

    def __getitem__(self, idx):
        x, y, mask = self.x[idx], self.y[idx], self.mask[idx]

        # x = Image.open(x).convert('L').convert('RGB')
        x = Image.open(x).convert('RGB')
        x = self.transform_x(x)
